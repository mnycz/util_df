#ifndef UTIL_DF_MATH_HH 
#define UTIL_DF_MATH_HH

// utility namespace for math functions
...
namespace util_df{
   namespace Math{
	 ...
	 T GetStandardDeviation(std::vector<T> x,int besselCor=Constants::kBesselsCorrectionDisabled){
      ...
      //______________________________________________________________________________
      template < typename T >
	 T GetStandardDeviation(int N,T *x,int besselCor=Constants::kBesselsCorrectionDisabled){
	    T var   = GetVariance<T>(N,x,besselCor);
	    T stdev = sqrt(var);
	    return stdev;
	 }
      //______________________________________________________________________________
